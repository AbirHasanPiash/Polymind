        ...
        <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl transition-all">
                  <div className="p-4 sm:p-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs font-medium text-slate-500 dark:text-gray-400 uppercase tracking-wider">Input Text</span>
                    </div>
                    {/* Textarea */}
                    <textarea
                      value={text}
              ...
              {lastGeneratedAudio && !isGenerating && (
                <div className="relative z-10 animate-in fade-in slide-in-from-top-2 duration-500">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-medium text-slate-500 dark:text-gray-400 uppercase tracking-wider">Just Generated</span>
                  </div>
            ...
            <div className="space-y-4 relative z-0">
              <div className="bg-white dark:bg-blue-900/10 backdrop-blur-sm p-5 sm:p-6 rounded-2xl border border-blue-100 dark:border-blue-800/20 shadow-sm dark:shadow-lg hover:border-blue-300 dark:hover:border-blue-500/30 transition-colors">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div>
                    <h4 className="text-slate-900 dark:text-white text-sm font-semibold mb-1">Studio Quality</h4>
              ...
              <div className="bg-white dark:bg-purple-900/10 backdrop-blur-sm p-5 sm:p-6 rounded-2xl border border-purple-100 dark:border-purple-800/20 shadow-sm dark:shadow-lg hover:border-purple-300 dark:hover:border-purple-500/30 transition-colors">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-slate-900 dark:text-white text-sm font-semibold mb-1">Instant Download</h4>
                    <p className="text-slate-500 dark:text-gray-400 text-xs leading-relaxed">
          ...
          <div className="relative z-0 mb-6">
            <div className="flex items-center gap-3 mb-6 border-b border-slate-200 dark:border-gray-800/50 pb-4">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Recent Generations</h2>
            </div>
            ...
            {!isLoading && audioFiles?.length === 0 && (
              <div className="text-center py-16 sm:py-24 bg-white/50 dark:bg-slate-900/30 backdrop-blur-sm rounded-2xl border border-dashed border-slate-300 dark:border-slate-800/50">
                ...
                <p className="text-slate-500 dark:text-gray-500 text-sm sm:text-base">No audio generated yet. Start creating!</p>
              </div>
            )}
...
function AudioCard({ 
  ...
  return (
    <div className={`group bg-white dark:bg-slate-900/50 backdrop-blur-sm border ${isHighlighted ? 'border-emerald-200 dark:border-green-500/50 shadow-lg shadow-emerald-500/10 dark:shadow-green-900/20' : 'border-slate-200 dark:border-slate-800/50 hover:border-blue-300 dark:hover:border-slate-700'} p-5 sm:p-6 rounded-2xl transition-all shadow-sm dark:shadow-lg hover:shadow-md hover:-translate-y-1 relative`}>
      ...
      <div className="flex justify-between items-start mb-4 pr-8">
        <div className="flex items-center gap-2">
          <div className={`w-9 h-9 rounded-full ${isHighlighted ? 'bg-gradient-to-br from-emerald-500 to-green-600' : 'bg-gradient-to-br from-blue-500 to-purple-600'} flex items-center justify-center shadow-md`}>
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <ChevronDownIcon className={`h-5 w-5 text-slate-400 dark:text-gray-400 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      
      {isOpen && (
        // Dropdown Styles
        <div className="absolute top-full mb-2 sm:mb-0 sm:mt-2 w-full overflow-auto rounded-xl bg-white dark:bg-[#0f1117] backdrop-blur-xl py-1 text-sm shadow-2xl border border-slate-200 dark:border-slate-700 z-[60] max-h-60 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] animate-in fade-in zoom-in-95 duration-200 origin-bottom sm:origin-top"
          style={{ bottom: window.innerWidth < 640 ? '100%' : 'auto' }}
        >
          {voices.map((voice) => (
            <button
              key={voice.id}
            ...
            >
              <span className={`block truncate ${selected.id === voice.id ? 'font-semibold text-blue-800 dark:text-white' : 'font-normal'}`}>
                <span className="text-slate-400 dark:text-gray-500 mr-2">{voice.name}</span>
              </span>
              {selected.id === voice.id && (
